...
func TestParseComment(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			comment:   "//nolint:musttag",
			wantMatch: true,
			wantRules: []string{"musttag"},
		},
		{
			name:      "nolint with multiple rules",
			comment:   "//nolint:musttag,wrapcheck",
			wantMatch: true,
			wantRules: []string{"musttag", "wrapcheck"},
		},
		{
			name:      "nolint with reason",
			comment:   "//nolint:musttag // uses mapstructure instead",
			wantMatch: true,
			wantRules: []string{"musttag"},
		},
		{
			name:      "nolint with hyphenated rule",
			comment:   "//nolint:revive-exported",
			wantMatch: true,
			wantRules: []string{"revive-exported"},
		},
		{
		...
		{
			name:      "comment documenting nolint syntax",
			comment:   "// nolintPattern matches // nolint directives",
			wantMatch: false,
			wantRules: nil,
		},
	}
...
func TestDirective_Match(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "empty rules do not suppress nolintlint",
			rules:  nil,
			check:  "nolintlint",
			expect: false,
		},
		...
		},
		{
			...
			rules:  []string{"musttag", "wrapcheck"},
			check:  "wrapcheck",
			expect: true,
		},
	}
